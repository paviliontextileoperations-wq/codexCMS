...
import { Barcode, Check, FileText, List, Minus, PackagePlus, Plus, Printer, Search, Trash2, Truck, UserPlus, Wallet, X } from "lucide-react";
...
export function POSView() {
  ...
  const [transportMethod, setTransportMethod] = useState<"PICKUP" | "DELIVERY" | null>(null);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        POS_DRAFT_KEY,
        JSON.stringify({ customer, lines, priceMode, customerQuery }),
      );
    } catch {
      // ignore quota errors
    }
  }, [customer, lines, priceMode, customerQuery]);
